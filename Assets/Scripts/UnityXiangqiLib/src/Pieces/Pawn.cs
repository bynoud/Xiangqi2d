...
namespace UnityXiangqi
{
	public class Pawn : Piece<Pawn> {
		...
		public override string ToText() => Owner==Side.White ? "P" : "p";

		protected override IEnumerable<Movement> EnumeratePossibleMoves (
			Board board,
			Square position
		) {
            List<Square> offsets = new List<Square> { new Square(0, Owner.ForwardDirection()) };
            if (riverCross(position)) {
				offsets.Add(new Square(1, 0));
				offsets.Add(new Square(-1, 0));
			}

			foreach (Square offset in offsets) {
