...
namespace UnityXiangqi
{
	public class FENSerializer : IGameSerializer {
		...
		private static (Square, Piece)[] GetPieces(string boardString) {
			...
			for (int i = 0; i < rankStrings.Length; ++i) {
				...
				foreach (char character in rankString) {
					...
					result.Add((new Square(file++, RANK_MAX - i), Piece.FromChar(character.ToString())));
				}
			}
		...
		private static string CalculateBoardString(Board currentBoard) {
			string[] rankStrings = new string[RANK_MAX];
			for (int rank = 1; rank <= RANK_MAX; rank++) {
				int emptySquareCount = 0;
				int rankStringsIndex = RANK_MAX - rank;
				rankStrings[rankStringsIndex] = "";
				for (int file = 1; file <= FILE_MAX; file++) {
					Piece piece = currentBoard[file, rank];
					if (piece == null) {
						emptySquareCount++;
						if (file == FILE_MAX) { // reached end of rank, append empty square count to rankString
						...
						rankStrings[rankStringsIndex] += piece.ToChar();
					}
				}
			}
